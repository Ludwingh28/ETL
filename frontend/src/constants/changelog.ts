...
export const CHANGELOG: ChangelogVersion[] = [
  {
    ...
    fixes: [
      ...
      "Dashboard Vendedor (privilegiados) — el listbox de selección de vendedor aparecía vacío para gerentes y administradores al entrar en septiembre porque el endpoint consultaba fact_ventas, que no tiene datos del mes en curso hasta el día siguiente (el ETL va un día detrás); corregido con un nuevo endpoint /dashboard/vendedores-nombres/ que consulta dw.dim_vendedor directamente (es_vendedor_actual = true), independiente del mes seleccionado; el selector de período sigue mostrando el mes en curso aunque los datos aún no estén disponibles",
    ],
    features: [
  ...
  },
  {
    ...
    fixes: [
      ...
      "Dashboard Vendedor — tabla de clientes se expandía horizontalmente desplazando el panel de detalle de SKUs fuera de la pantalla; el layout ahora es vertical (space-y-4) de modo que el panel de SKUs aparece debajo de la grilla de clientes",
      "Dashboard Vendedor — panel de filtros se superponía al navbar de móvil al hacer scroll: z-index del panel bajado de z-30 a z-20, por debajo del navbar (z-40) y del menú móvil (z-30)",
      "Dashboard Vendedor — dropdown de selección de vendedor se desbordaba fuera de la pantalla en móvil al abrirse hacia la derecha; corregido con left-0 en móvil y max-w-[calc(100vw-1rem)] para que nunca exceda el ancho del viewport",
      "Dashboard Vendedor — cartera disminuía al seleccionar una marca o categoría porque la consulta incluía filtros de producto; la cartera (clientes únicos anuales) es ahora independiente de los filtros de producto",
      "Dashboard Vendedor — fecha_corte cambiaba al seleccionar una marca o categoría porque se calculaba en la misma query con filtros de producto; movida a un query separado sql_fc sin filtros de producto",
      "Dashboard Vendedor — filtro de rutas no coincidía con ruta_descripcion de fact_ventas; las rutas se obtienen ahora de dual.dim_planificacion y el filtro usa la cadena de join dim_planificacion → dim_cliente_dual → dim_cliente → fact_ventas en lugar de filtrar directamente por ruta_descripcion",
      "Administración — Dashboard Vendedor (vendedores-personal) no aparecía en el panel de creación/edición de usuarios porque faltaba en la lista DASHBOARD_GROUPS de adminConstants; agregado en el grupo 'Mock-ups'; cargo 'Vendedor' ahora incluye ese permiso por defecto",
      "Backend — usuarios con permiso vendedores-personal recibían 403 al cargar el dashboard porque los endpoints /dashboard/nacional/kpis/, /dashboard/nacional/tendencia/, /dashboard/new-nacional/opciones/, /comparacion/, /skus/ y /cliente-fechas/ solo aceptaban los permisos 'nacional' o 'new-nacional'; el decorator _require_perm ahora acepta múltiples permisos (OR) y cada endpoint relevante fue actualizado para aceptar también 'vendedores-personal'",
      "Dashboard Vendedor — supervisores, gerentes (General, de Ventas, Regional) y analistas de datos quedaban bloqueados a su propio perfil sin poder elegir otro vendedor; la lista de cargos privilegiados (PRIVILEGED_CARGOS) tenía el valor incorrecto 'Gerente' en lugar de 'Gerente General' y 'Gerente de Ventas', y no incluía 'Supervisor' ni 'Analista de Datos'; corregido con los nombres exactos de cargo",
    ],
    features: [
      ...
      "Mock-up — tabla Por Cliente (Nacional): fila de totales fija al pie de la grilla con total por columna de fecha y total general; celdas de esquina sticky con z-index más alto para no solaparse al hacer scroll diagonal",
      ...
      "Dashboard Vendedor — tabla Por Cliente rediseñada como grilla de fechas (idéntica al Mock-up Nacional): columnas por día de compra, drill por fecha, totales fijos al pie por columna de fecha y total general",
      "Dashboard Vendedor — panel de filtros: botón de fijar/desfijar (Pin/PinOff) que ancla el panel encima del contenido al hacer scroll; en móvil los filtros se distribuyen en cuadrícula 2×3 en lugar de lista vertical; en escritorio se mantiene el layout flex original",
      "Dashboard Vendedor — KPI 'Unidades Vendidas': muestra unidades vendidas vs unidades presupuestadas (ppto_uds) como referencia adicional",
      "Dashboard Vendedor — KPI 'Clientes Activos': muestra la cartera base del vendedor y, en línea separada, el % de cobertura (clientes con compra vs cartera) con color semáforo verde/amarillo/rojo",
      "Dashboard Vendedor — KPI 4 cambiado de '% Cumplimiento' a 'vs Mes Anterior': muestra el importe del mes anterior y el % de cambio mes a mes (MoM) en color verde si es alza, rojo si es baja",
      "Dashboard Vendedor — rutas obtenidas de dual.dim_planificacion en lugar de fact_ventas.ruta_descripcion; cuando se selecciona una ruta la cartera muestra los clientes asignados en dim_planificacion para esa ruta (no el total anual); el KPI 'Ventas del Mes' cambia su título a 'Ventas Ruta(s)', el ppto de referencia pasa a ser el total vendido del vendedor y el % muestra la participación de la ruta sobre el total",
      "Dashboard Vendedor — tablas de SKU (principal y panel de detalle por cliente): muestran el código de producto como primera columna; el buscador de SKUs también filtra por código además de nombre",
    ],
  ...
  },
  {
    ...
    features: [
      ...
      "Dashboard Vendedor — inicio del desarrollo: estructura base con KPIs, tendencia, comparación por categoría/proveedor/marca, tabla de SKUs con drill interactivo y tabla de clientes; el lanzamiento oficial con permisos operativos y funcionalidades completas se publica en la versión 1.2.9.0",
    ],
    newDashboardPerms: [],
    newDashboardNames: {},
  },
