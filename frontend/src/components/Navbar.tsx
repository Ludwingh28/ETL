          ...
          return (
            <NavLink
              ...
              className={({ isActive }) => `
                ...
                ${isActive ? "bg-brand-50 text-brand-700 font-semibold" : "text-slate-700 hover:bg-slate-50 hover:text-slate-900"}
              `}
            >
      ...
      {open && (
        <div className="absolute right-0 top-[calc(100%+0.5rem)] z-50 min-w-52 bg-white rounded-xl shadow-2xl border border-slate-200 py-1.5">
          ...
          <div className="px-4 py-2.5 border-b border-slate-100">
            ...
            {(user?.groups?.length ?? 0) > 0 && <p className="text-xs text-brand-600 mt-0.5">{user?.groups.join(", ")}</p>}
          </div>
      ...
      >
        <div className="px-4 py-3 space-y-1">
          {visibleGroups.map((group) => (
            <div key={group.label}>
              <p className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold px-3 py-1.5">{group.label}</p>
              {group.items.map((item) => {
                const Icon = item.icon;
                return (
                  <NavLink
          ...
          <div className="border-t border-white/10 pt-3 mt-2">
            <p className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold px-3 py-1.5">Mi Cuenta</p>
            {userMenuItems.map(({ to, icon: Icon, label }) => (
              <NavLink
                key={to}
                to={to}
